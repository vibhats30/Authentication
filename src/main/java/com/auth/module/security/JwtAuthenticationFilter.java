...
import com.auth.module.service.CustomUserDetailsService;
        ...
        try {
            ...
            if (StringUtils.hasText(jwt)) {
                ...
                if (tokenProvider.validateToken(jwt)) {
                    ...
                    logger.debug("Valid JWT token for userId: {} accessing {}", userId, requestURI);
                    ...
                    logger.debug("User authentication set in security context for userId: {}", userId);
                } else {
                    logger.warn("Invalid JWT token for request: {}", requestURI);
                }
            } else {
                logger.debug("No JWT token found in request to {} - proceeding without authentication", requestURI);
            }
        } catch (Exception ex) {
